...
import sys, os
sys.path.append('C:/Users/shraw/Python Workspace/lid/Spoken-language-identification-master/theano/networks')
...
class Network(BaseNetwork):
    ...
    def __init__(self, train_list_raw, test_list_raw, png_folder, batch_size, dropout, l2, mode, batch_norm, **kwargs):
        ...
        example = np.random.uniform(size=(self.batch_size, 1, 128, 400), low=0.0, high=1.0).astype(np.float32) #########
        answer = np.random.randint(low=0, high=176, size=(self.batch_size,)) #########
       
        network = layers.InputLayer(shape=(None, 1, 128, 400), input_var=self.input_var)
    ...
    def read_batch(self, data_raw, batch_index):
        ...
        data = np.zeros((self.batch_size, 1, 128, 400), dtype=np.float32)
        answers = []
        ...
        for i in range(start_index, end_index):
            ...
            im = Image.open(path)
            im = im.resize((400,128), Image.ANTIALIAS)
